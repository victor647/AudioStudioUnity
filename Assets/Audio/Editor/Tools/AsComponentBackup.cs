...
{   
    public class AsComponentBackup : AsSearchers
    {
        ...
        public delegate bool ComponentImporter(AsComponent component, XElement node);
        private delegate void ComponentExporter(AsComponent component, XElement node);  
        private Dictionary<Type, bool> _componentsInSearch = new Dictionary<Type, bool>();
        ...
        private bool _separateXmlFiles = true;
        ...
        protected override string DefaultXmlPath 
        {
            get
            {
                return AudioUtility.CombinePath(XmlDocPath, "AudioStudioComponents.xml");        
            }
        }
        ...
        private void OnGUI()
        {
            GUILayout.Label("This tool searches for AudioStudio components and export data to an xml file. \n" +
                            "It can also import data from xml and place AudioStudio components into the game.");
            using (new EditorGUILayout.VerticalScope(GUI.skin.box))
            {
                using (new EditorGUILayout.HorizontalScope(GUI.skin.box))
                {
                    ...
                    if (GUILayout.Button("Select All", GUILayout.Width(120))) SelectAllToggles(true);
                    if (GUILayout.Button("Deselect All", GUILayout.Width(120))) SelectAllToggles(false);
                ...
                {                                                            
                    ...
                    _separateXmlFiles = GUILayout.Toggle(_separateXmlFiles, "Export Separate Xml Files");
                }
        ...
        {
            var selections = new Dictionary<Type, bool>(_componentsInSearch);
            foreach (var selection in selections)
            {
                var selected = GUILayout.Toggle(selection.Value, selection.Key.Name);
                if (selected != selection.Value)
                    _componentsInSearch[selection.Key] = selected;
            }
        }

        private void SelectAllToggles(bool select)
        {
            var selections = new Dictionary<Type, bool>(_componentsInSearch);
            foreach (var selection in selections)
            {
                _componentsInSearch[selection.Key] = select;
            }
        }
        #endregion              
        ...
        private void Export()
        {
            ...
            var fileName = "";
            CleanUp();
            if (_separateXmlFiles)
            {
                foreach (var component in _componentsInSearch)
                {
                    if (component.Value)                                            
        ...
        public void AutoExport(string searchPath, string xmlPath, Type type = null)
        {
            ...
            if (type != null)
            {
                _componentsInSearch.Clear();
                _componentsInSearch[type] = true;
            }
        ...
        {                                    
            ...
            foreach (var component in components)
            {
                ...
                if (!_componentsInSearch.ContainsKey(type) || !_componentsInSearch[type]) continue;
                if (_separateXmlFiles)                
        ...
        public void RemoveComponentNode(AsComponent component)
        {
            var type = component.GetType();
        ...
        public void RevertPrefab(string prefabName)
        {
            CleanUp();                        
            LoadOrCreateXmlDoc();       
            ...
            foreach (var xComponent in xComponents)
            {
                ...
                if (prefab == null)
                {
                    UnityEngine.Debug.LogWarning("Import Failed: Can't find prefab at " + prefabPath);
                    continue;
                }                
                var objName = AudioUtility.GetXmlAttribute(xComponent, "GameObject");
        ...
        private void ImportFromPrefabs(bool isCompare)
        {
            try
            {
                ...
                for (var i = 0; i < xComponents.Count; i++)
                {
                    ...
                    if (prefab == null)
                    {
                        UnityEngine.Debug.LogWarning("Import Failed: Can't find prefab at " + fullPath);
                        continue;
                    }
        ...
        {
            var type = component.GetType();
            if(Importers[type] != null && _componentsInSearch[type])
            {                
        ...
        #region Combine
        public void CombineSeparateXml()
        {
            CleanUp();
            foreach (var type in _componentsInSearch.Keys)
            {                
                LoadOrCreateXmlDoc(type);
            }            
        ...
        {               
            ...
            {                        
                ...
                if (CheckError(component)) return;
            ...
            private bool CheckError(AsComponent component)
            {
                if (!component)
                {
