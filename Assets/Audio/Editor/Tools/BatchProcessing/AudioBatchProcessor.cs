...
{        
    ...
    {		
        ...
        private void GenerateMusicInstruments(bool saveAssets)
        {
            try
            {
                var searchPath = Path.Combine(Application.dataPath, AudioPathSettings.OriginalsPath, "Instruments");
                ...
                for (var i = 0; i < samplePaths.Length; i++)
                {
                    ...
                    var savePath = $"Assets/{AudioPathSettings.MusicInstrumentsPath}/{instrumentName}.asset";
                    ...
                    if (!mapping.Samples.Contains(sample))
                        AudioUtility.AddToArray(ref mapping.Samples, sample);
        ...
        private static void CleanUp(AudioConfig ae)
        {
            ae.OnValidate();
            ae.CleanUp();         
