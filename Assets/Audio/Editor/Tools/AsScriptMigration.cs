...
namespace AudioStudio.Tools
{
	...
	public class AsScriptMigration : EditorWindow
	{
		...
		private string _thisGameRootPath;
		...
		private void SearchScriptDifference()
		{
			...
			foreach (var path in _sourceScripts)
			{
				...
				{
					SourceFilePath = path,
					TargetFilePath = _migrateToOtherGame ? path.Replace(_thisGameRootPath, _otherGameRootPath) : 	path.Replace(_otherGameRootPath, _thisGameRootPath),					 
			...
			EditorUtility.DisplayDialog("Check Result", _changeList.Count == 0 ? "All files are identical!" : _changeList.Count + " files are different.", "OK");
		}
		...
		private void GetAllScripts()
		{
			...
			var thisScript = new FileInfo( scriptFilePath);
			_thisGameRootPath = thisScript.Directory.Parent.Parent.FullName;
			_sourceScripts = Directory.GetFiles(_migrateToOtherGame ? _thisGameRootPath : _otherGameRootPath, "*.cs", SearchOption.AllDirectories);
		}
