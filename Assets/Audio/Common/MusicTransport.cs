...
namespace AudioStudio
{
    ...
    public class MusicTransport : MonoBehaviour
    {
        ...
        {
            get { return ActiveMusicData.RemainingLoops; }
        ...
        private int ExitPositionSamples => Mathf.FloorToInt(ActiveMusicData.MusicTrack.LoopDurationRealTime() * SampleRate);
        ...
        public void SetMusicQueue(MusicContainer evt, float fadeInTime, float fadeOutTime, float exitOffset = -1f, float entryOffset = -1f)
        {            
        ...
        {            
            ...
            {
                var mti = track.CreateChildEmitter<MusicTrackInstance>(gameObject);                     
                mti.Init(track);
        ...
        public void Stop(float fadeOutTime)
        {
            ...
            foreach (var msi in PlayingTrackInstances)
            {
                if (msi) msi.Stop(fadeOutTime);                                
            }
            foreach (var esi in _exitingTrackInstances)
            {
        ...
        private void PrepareTransition(float fadeInTime, float fadeOutTime, float exitOffset, float entryOffset)
        {
            ...
            var segment = AudioAssetLoader.GetTransitionSegment(CurrentPlayingEvent, NextPlayingEvent);
            if (segment)
            {
        ...
        public void SetSequence(MusicContainer nextMusic) //get the next item of a sequence continuous music container
        {
